...
public:
  /*
  emails - Vector of String that needs to be parsed
  */
  int numUniqueEmails(vector<string>& emails)
  {
    int num = 0;
    vector<string> NumberofMails;
    for (auto& email : emails)
    {
      ...
      string actual = local + '@' + domain;
      if (NumberofMails.empty())
      {
        ++num;
        NumberofMails.push_back(actual);
      }

      bool exist = false;
      for (auto& a : NumberofMails)
      {
        if (actual == a)
          exist = true;
      }
